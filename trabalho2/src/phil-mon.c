...
void setTheTable() {
	...
	pthread_cond_init(&forkLock, NULL);
}
...
void philGrabForks(Philosopher *p) {
	...
	while(!philCanEat(p)) 
	  pthread_cond_wait(&forkLock, p->mutex);

	// grabs forks
	pthread_cond_wait(p->forkLeft, p->mutex);
	pthread_cond_wait(p->forkRight, p->mutex);
